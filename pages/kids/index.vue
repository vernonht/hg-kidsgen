      ...
      <a-spin class="w-full" tip="Loading..." :spinning="loading">
          ...
          <div class="flex flex-col md:flex-row bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg mb-4 trans" v-for="(item, index) in kids">
              ...
              <div class="md:w-3/4 p-6 my-auto">
                  <kid-info :kid="item" :parents="parents" :loading="loading" @setLoading="setLoading" @updated="getKidsListing"/>

                  <div class="text-left grid grid-cols-5 gap-2">
                      <button class="button col-span-5 md:col-span-2 lg:col-span-1" @click="manageQRModal(item)">
                          <a-icon type="qrcode"/>
                          <span class="pl-2">Show QR</span>
                      </button>
                      <button class="button col-span-5 md:col-span-2 lg:col-span-1" @click="attendanceModal(item)">
                          <a-icon type="check-square" theme="twoTone" twoToneColor="#52c41a"/>
                          <span class="pl-2">Attendance</span>
                      </button>
                      <button class="button col-span-5 md:col-span-2 lg:col-span-1" @click="$router.push(`/kids/${item.id}`)">
                          <a-icon type="edit"/>
                          <span class="pl-2">Edit Info</span>
                      </button>
                      <a-popconfirm :title="`Delete ${item.name}?`" @confirm="deleteKid(item)" okText="Yes" cancelText="No">
                          <button class="button col-span-5 md:col-span-2 lg:col-span-1">
